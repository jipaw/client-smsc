...
function Modem (opts) {
  ...
  this.forever = opts.forever || false
...
Modem.prototype.addMsidn = function (number) {
  number.msidn = []
  return number.msidn.push(this.msidn)
}

Modem.prototype.addSetting = function (number) {
  number.setting = []
  obj = {
    group: this.group,
    delay: this.delay
  }
  return number.setting.push(obj)
}
  ...
  if (arr.length > 0) {
    ...
    if (port === this.dataPort && this.commandsStack.length > 0) {
      ...
      if (lastLine.indexOf('ERROR') !== -1 || lastLine.indexOf('NOT SUPPORT') !== -1) {
        ...
        if (resp.indexOf(cmd.waitCommand) !== -1) {
          b_Finished = true
        }
...
Modem.prototype.handleNotification = function (line) {
  ...
  if (line.substr(0, 5) === '+CMTI') {
      ...
      this.getSMS(storage, smsId, function (err, msg) {
        if (err === undefined) {
          ...
          if (msg.udh && msg.udh.parts && msg.udh.parts > 1) {
            ...
            this.addMsidn(msg)
            this.emit('message', msg)
            // console.log('LOG MSIDN FROM LIBARARY' + this.msidn);
            this.addMsidn(msg)
    ...
    if (match !== null && match.length > 2) {
     ...
     //      var trackingObj = _.has(this.deliveryParts[msg.reference]);
     //      if(trackingObj) {
     //        trackingObj.parts--;
     ...
     //            0x49  Does not exist
     //            // reportObj.msidn = this.msidn; // add msidn property
     ...
     //        });
     //        trackingObj = null;
	    // // console.log(this.msidn);
     //        this.addMsidn(reportObj)
    ...
    match = line.match(/\+CUSD:.(1|4)([\s\S]*)/)
    if (match !== null && match.length === 3) {
      handled = true
      this.emit('USSD', parseInt(match[1]), match[0], match[2])
    }
